...
function getMsg(i,origin){
	return `error value at "${origin.slice(Math.max(i-20,0),i+1).join('').replace(/\s/g,'')}"`
}
let CommonErrors={}
Object.entries(CommonCodes).forEach(([key,value])=>{
    CommonErrors[key]=new Error(value.msg)
})
class Scan {
    constructor(origin) {
        this.origin=origin
    ...
    stateBeginObjectKey(c) {
        ...
        default: {
            if (/[a-zA-Z_]/.test(c)) {
                ...
                console.log(c)
                throw CommonErrors.InvalidObjectKey
            }
        }
        }
    }
